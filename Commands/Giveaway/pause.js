...
const { pauseGiveaway, getGiveaway } = require('../../giveaway');
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const success = pauseGiveaway(messageId);
    if (!success) {
      return interaction.reply({ content: 'Giveaway not found or already paused.', flags: 64 });
    }
      ...
      if (giveaway) {
        ...
        const container = new ContainerBuilder()
          ...
          .addTextDisplayComponents(
            new TextDisplayBuilder().setContent(`# 🎉 Giveaway (Paused)\n\n**Prize:** ${giveaway.prize}\n**Ends:** <t:${Math.floor(giveaway.endTime / 1000)}:R>\n**Hosted by:** <@${giveaway.hostId}>`)
          );
    ...
    await interaction.reply({ content: 'Giveaway paused.', flags: 64 });
  }
};